...
class AIService:
    ...
    async def _complete(self, **kwargs) -> str:
        try:
            ...
            raise AIServiceError(f"AI provider request failed: {type(exc).__name__}: {exc}") from exc

    async def summarize(self, ticket: Ticket) -> str:
        system, user = prompts.summarize_prompt(ticket)
        text = await self._complete(system=system, user=user, temperature=0.3, max_tokens=300)
        return text.strip()
    ...
    async def suggest_reply(self, ticket: Ticket, instructions: str | None = None) -> str:
        ...
        text = await self._complete(system=system, user=user, temperature=0.5, max_tokens=400)
        return text.strip()

    async def generate_internal_note(self, ticket: Ticket) -> str:
        system, user = prompts.internal_note_prompt(ticket)
        text = await self._complete(system=system, user=user, temperature=0.3, max_tokens=200)
        return text.strip()
    ...
    async def answer_with_rag(self, question: str, articles: list[dict]) -> str:
        ...
        text = await self._complete(system=system, user=user, temperature=0.2, max_tokens=600)
        return text.strip()
